...
class LeVADocumentUseCaseSpec extends SpecHelper {
    ...
    def "create SSDS"() {
        ...
        def resource = new FixtureHelper().getResource("Test.docx")
        def sqReportFiles = [[name: resource.name, path: resource.path, directory: false,
                              length: resource.size(), lastModified: resource.lastModified()]]
